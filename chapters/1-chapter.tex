...
A câmera que torna isso possível é um dispositivo da Intelbras, modelo VIP 5460 LPR IA, cujo valor de mercado varia entre R\$ 5.700 e R\$ 7.500, longe de um preço acessível para aplicações de menor escala ou casos em que a quantidade de dispositivos necessários é maior.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{images/rock3a.png}
...
Para que essa divisão da placa em caracteres tenha sucesso, é necessário saber como os caracteres estão dispostos pela placa, o que varia conforme o tipo da placa. No Brasil existem dois de \textit{layout} de placa permitidos: O tipo antigo, ainda em circulação, e o tipo Mercosul, que passou a ser obrigatório para veículos novos no Brasil em 2020.
...
Uma outra possibilidade é tratar o problema de extração e classificação dos caracteres como um único problema a ser resolvido por um modelo mais robusto de inteligência artificial que realiza as duas tarefas de uma só vez. Essa alternativa é mais simples mas pode gerar bons resultados se a quantidade de dados disponível para ajuste do modelo for suficiente.

Os diagramas das figuras \ref{fig:pipeline} e \ref{fig:pipelinesimples} demonstram as etapas do algoritmo em ambas as metodologias. O método 1 separa a extração e classificação dos caracteres e o método 2 os executa juntos.

\begin{figure}[H]
    \centering
    \begin{minipage}{0.47\textwidth}
        \centering
        \includegraphics[width=0.8\linewidth]{images/diagrama_pipeline.png}
        \caption{Diagrama das etapas da leitura de placas (método 1)}
        \label{fig:pipeline}
    \end{minipage}
    \hfill
    \begin{minipage}{0.47\textwidth}
        \centering
        \includegraphics[width=0.8\linewidth]{images/pipeline_simples.png}
        \caption{Diagrama das etapas da leitura de placas (método 2)}
        \label{fig:pipelinesimples}
    \end{minipage}
\end{figure}
...
\begin{enumerate}
	...
	\item Geração de um \textit{dataset} de caracteres extraídos e um \textit{dataset} de placas extraídas
...
\subsection{Desenvolvimento de algoritmo de pré-processamento e extração de caracteres}
...
\subsection{Geração de \textit{datasets} de caracteres e placas}

Para que seja possível treinar os modelos de classificação dos caracteres é necessário criar \textit{datasets} correspondentes aos modelos que se deseja ajustar. No caso dos modelos que realizam classificação de caracteres já extraídos é necessário criar um \textit{dataset} de imagens de caracteres. Para isso, tendo sido realizadas as etapas anteriores, basta aplicar a detecção de placa e extração de caracteres em cada imagem dos dados de treinamento. Como sabemos previamente quais placas são do tipo Brasil e quais são Mercosul, assim como também sabemos quais caracteres são números e quais são letras, é possível criar \textit{datasets} separados para cada categoria, resultando em quatro \textit{datasets}:
...
Já no caso dos modelos de classificação e extração de caracteres, o \textit{dataset} de treinamento não pode conter caracteres extraídos, e sim placas de carro extraídas das imagens originais a partir do \textit{output} do modelo de detecção de placas. Para esse modelo, muito do processamento realizado nas imagens de placas não é necessário, mas a correção de perspectiva ainda é interessante de ser utilizada para corrigir distorções e padronizar as dimensões das imagens de placas.

O \textit{dataset} de placas, portanto, é formado por imagens de placas com dimensões fixas, que podem ainda ser divididos em:

\begin{itemize}
	\item Placas Brasil
	\item Placas Mercosul
\end{itemize}

\subsection{Treinamento de modelos de classificação de caracteres}

O treinamento dos modelos de classificação de caracteres é dividido em duas partes, uma para a criação dos modelos de classificação de caracteres extraídos e outra para os modelos de extração e classificação. Os modelos de caracteres extraídos utilizam o \textit{dataset} de imagens de caracteres para seu ajuste, enquanto os modelos de extração e classificação utilizam os \textit{datasets} de placas.

A tarefa de classificar caracteres isolados não é particularmente complexa, portanto uma rede neural convolucional simples já é suficiente para esse problema. Como os dados de caracteres estão separados por modelo de placa e por tipo (letra/número) foram criados quatro modelos diferentes, um para cada combinação. Todos possuem a mesma arquitetura, exceto pela camada de saída, que possui 26 classes nos modelos de letras e 10 classes nos modelos de números.

Os modelos de extração e classificação, por outro lado, deve ser muito mais complexos, pois são incumbidos de uma tarefa muito mais difícil. Esses modelos devem receber na entrada uma imagem de uma placa de carro e convertê-la em 7 caracteres, cada um sendo representado por um vetor de 26 ou 10 valores, dependendo se aquele caractere é uma letra ou um número.

Como existem dois modelos de placa e cada um possui um padrão de letras e números, foram criados dois modelos dessa modalidade, um para cada padrão de placa.

\subsection{Conversão dos modelos para formato compatível com NPU}

O formato compatível com a NPU do ROCK 3A é o formato RKNN. A própria documentação da placa indica uma biblioteca chamada \textit{rknn toolkit} que oferece a \textit{API} tanto para conversão dos modelos para esse formato quanto para inferência com esses modelos. Porém, a conversão não é tão trivial, pois precisa ser feita em um ambiente \textit{linux} de uma determinada distribuição e versão. Além disso, os modelos precisam ser exportados para o formato ONNX antes de serem convertidos, pois o \textit{rknn toolkit} faz a conversão entre esses dois formatos.

Além desses requisitos obrigatórios para a conversão, existe um requisito opcional, porém recomendado. Este é que durante a conversão para RKNN os modelos sejam quantizados para \textit{uint8}, ou seja, que seus pesos sejam convertidos de números reais do tipo \textit{float} de 32 \textit{bits} para números inteiros positivos de 8 \textit{bits}. Isso é recomendado pois intensifica a performance da NPU.
